...
for epoch in range(args.num_epochs):
    ...
    if (epoch + 1) % args.log_epochs == 0:
        ...
        if args.flag_ttest:
            ...
            fake_ps = 0
            fake_rs = 0
            fake_preds, fake_pval, _  = fake_ttester.get_bh_correction(cns_np)
            print("GAN-generated #Rejects: {:d}".format(np.sum(fake_preds)))
            fake_p, fake_r = get_ttest(targets, fake_preds)
            fake_ps += fake_p
            fake_rs += fake_r
