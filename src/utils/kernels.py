...
def mix_rbf_rep(X, Y, sigma_list, _lambda):
    K_XX, K_XY, K_YY, d = _mix_rbf_kernel(X, Y, sigma_list)
    # return _mmd2(K_XX, K_XY, K_YY, const_diagonal=d, biased=biased)
    return _rep(K_XX, K_XY, K_YY, _lambda)

def _rep(K_XX, K_XY, K_YY, _lambda, biased=True):
    ...
    if biased:
        rep = (_lambda * (Kt_XX_sum + sum_diag_X) / (m * m)
            - (Kt_YY_sum + sum_diag_Y) / (m * m)
            - (_lambda - 1) * K_XY_sum / (m * m))
    else:
        rep = (_lambda * Kt_XX_sum / (m * (m - 1))
                - Kt_YY_sum / (m * (m - 1))
                - (_lambda - 1) * K_XY_sum / (m * m))
